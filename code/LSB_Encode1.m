    ...
    for j = 1:n
               ...
               if bitget(BinImg(i,j),1) == 0 && bitget(CarrierImg(i+m,j),1) ==1
                ...
                CarrierImg(i+m,j+m) = CarrierImg(i+m,j+m)-2;
                 ...
                 if bitget(BinImg(i,j),7) == 0 && bitget(CarrierImg(i+m,j+m),3) ==1
                  CarrierImg(i+m,j+m) = CarrierImg(i+m,j+m)-4;
                  elseif bitget(BinImg(i,j),7) == 1 && bitget(CarrierImg(i+m,j+m),3) ==0
                  CarrierImg(i+m,j+m) = CarrierImg(i+m,j+m)+4;
                  end
                
                  if bitget(BinImg(i,j),8)  == 1 && bitget(CarrierImg(i+m,j+m),4) ==0
                  CarrierImg(i+m,j+m) = CarrierImg(i+m,j+m)+8;
                 elseif bitget(BinImg(i,j),8) == 0 && bitget(CarrierImg(i+m,j+m),4) ==1
                  CarrierImg(i+m,j+m) = CarrierImg(i+m,j+m)-8;
                 end
