%SYML_DMO   Demo script on least asymmetric Daubechies' filters
%
%           SYML_DMO is a demo script that illustrates the generation
%           of "symlets" that lead to wavelet bases. It also includes
%	    a demo for the cascade algorithm used to obtain a discrete
%	    approximation of the continuous scale and wavelet functions,
%	    and wavelet packet function too.
%   
%           SYML_DMO may be accessed from the 'Filter Design' menu
%	    section of WTDEMO, choosing the 'Symlets' option.
...
% We will generate a set of "least asymmetric" Daubechies' filters using
% the SYMLETS function. The argument of this function is the number of
% coefficients (the same for all filters). It must be an even number:

         [h,g,rh,rg]=symlets(16);
...
% (number of coefficients)/2, as the Daubechies filters.

% Press any key to continue ...
pause
%--------------------------------------------------------

% These filters lead to a scale and wavelet function. This solution
% solution comes from a selection of filters with maximum number of 
% vanishing moments and closest to linear phase. This choice of phase
% leads to the least asymmetric scale function, as you can observe
% in the next figure:
